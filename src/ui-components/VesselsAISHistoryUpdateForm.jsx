...
export default function VesselsAISHistoryUpdateForm(props) {
  const {
    id: idProp,
    vesselsAISHistory: vesselsAISHistoryModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = vesselsAISHistoryRecord
      ? { ...initialValues, ...vesselsAISHistoryRecord }
      : initialValues;
    setImo(cleanValues.imo);
    setVessel_name(cleanValues.vessel_name);
    setTimestamp(cleanValues.timestamp);
    setLatitude(cleanValues.latitude);
    setLongitude(cleanValues.longitude);
    setErrors({});
  };
  const [vesselsAISHistoryRecord, setVesselsAISHistoryRecord] = React.useState(
    vesselsAISHistoryModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(VesselsAISHistory, idProp)
        : vesselsAISHistoryModelProp;
      setVesselsAISHistoryRecord(record);
    };
    queryData();
  }, [idProp, vesselsAISHistoryModelProp]);
  React.useEffect(resetStateValues, [vesselsAISHistoryRecord]);
  const validations = {
    imo: [],
    vessel_name: [],
    timestamp: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            VesselsAISHistory.copyOf(vesselsAISHistoryRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "VesselsAISHistoryUpdateForm")}
      {...rest}
    >
      <TextField
        label="Imo"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || vesselsAISHistoryModelProp)}
